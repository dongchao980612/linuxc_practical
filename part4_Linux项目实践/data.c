...
// 以-1为分割 在这之前的所有数字都是需要给所有peer发送have消息的piece索引
...
int create_btcache()
{
	...
	// 分配无法被piece整除的剩余的部分的内存(last_piece)
	while(count > 0) {
		node = initialize_btcache_node();
		if(node == NULL) {
			printf("%s:%d create_btcache error\n",__FILE__,__LINE__);
...
// 该函数非常类似于 write_btcache_node_to_harddisk
...
// sequnce是存放该piece的第一个cache（slice）的下标
int write_piece_to_harddisk(int sequnce,Peer *peer)
{
	...
	while(slice_count > 0) {
		...
		Request_piece *req_q = peer->Request_piece_head;  // q代表上一个
		while(req_p != NULL) {
...
int release_read_btcache_node(int base_count)
{
	...
	Btcache           *q = NULL;
	int            count = 0;
	int       used_count = 0;  // 一个piece的access count总数
	int      slice_count = piece_length / (16*1024);
...
int is_a_complete_piece(int index, int *sequnce)
{
	...
	while(p != NULL) {
		if( count%slice_count==0 && p->index!=index ) { // 快速跳过无关piece
			num = slice_count;
						   ...
						   unsigned char *buff,int len,Peer *peer)
{
	...
	// p为空说明当前slice是其所在的piece的**第一块**下载到的数据
	...
	while(i > 0) {
		...
		if(i == 4) write_btcache_to_harddisk(peer);  // 将所有已经彻底下载完成的piece写入硬盘
