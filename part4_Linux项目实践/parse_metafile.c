...
int read_metafile( char* metafile_name ) {
    ...
    // 以二进制、只读的方式打开文件（因为piece的hash中可能会含有会被误认为字符串结束的0x00）
...
int read_announce_list() {
    ...
    if ( find_keyword( "13:announce-list", &i ) == 0 ) {
        if ( find_keyword( "8:announce", &i ) == 1 ) {
            ...
            while ( isdigit( metafile_content[ i ] ) ) {  // 获取字符串长度
        ...
        i++;                                      // skip 'l'
        while ( metafile_content[ i ] != 'e' ) {  // e -> end
            i++;                                  // skip 'l'
...
int get_info_hash() {
    ...
    for ( ; i < filesize; )
        ...
        else if ( metafile_content[ i ] == 'i' ) {  // skip i...e
            i++;                                    // skip i
            if ( i == filesize )
                return -1;
            while ( metafile_content[ i ] != 'e' ) {
        ...
        else if ( ( metafile_content[ i ] >= '0' ) && ( metafile_content[ i ] <= '9' ) ) {  // skip str
            int number = 0;
...
int parse_metafile( char* metafile ) {
    ...
    // 读取种子文件  返回到全局变量  metafile_content
    ...
    // 从种子文件中获取tracker服务器的地址 返回到全局变量 announce_list_head
