...
extern int                 *valid;   // 数组 1表示第i个 traker 连接成功
...
// buffer 指向 Traker 返回的消息  Traker 返回 0 1 两种消息类型
// len 表示 buffer 的总长度（不一定填满）
// total_length 用来接收消息的真实长度
// 0代表 peers 是字符串格式
// 1代表 peers 是字典列表格式
...
// 对每个 Traker 建立一个 socket, 并且 connect
int prepare_connect_tracker(int *max_sockfd)
{
	...
	tracker_count = count;  // traker 的总数
